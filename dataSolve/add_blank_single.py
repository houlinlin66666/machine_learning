...
def copy_excel_with_custom_suffix(source_path, target_folder, suffix_part):
    """
    复制单个Excel文件并按规则重命名（如：原名称-1-1）

    :param source_path: 源文件完整路径
    :param target_folder: 目标文件夹路径
    :param suffix_part: 后缀部分（如 "1-1"、"5-2"）
    :return: 是否复制成功
    """
    filename = os.path.basename(source_path)

    # 匹配新的文件名模式：X(FD3).xlsx（X为数字）
    match = re.match(r'^(\d+)\((FD3)\)\.xlsx$', filename)
    if not match:
        print(f"警告：{filename} 不符合命名格式，跳过该文件")
        return False

    base_part = match.group(1)  # 提取数字部分（如"10"）
    fd3_part = match.group(2)  # 提取"FD3"部分

    # 构建新文件名：X-1-1(FD3).xlsx（如10-1-1(FD3).xlsx）
...
def batch_copy_excel_custom_suffixes(source_dir, target_dir):
    """
    批量复制Excel文件，生成指定后缀的副本（1-1、1-2、5-1、5-2、10-1、10-2）
    """
    # 检查源文件夹
    ...
    excel_files = [
        ...
        if f.endswith('.xlsx') and re.match(r'^\d+\(FD3\)\.xlsx$', f)
    ]

    if not excel_files:
        print(f"警告：源文件夹 {source_dir} 中未找到符合格式的Excel文件")
        print("符合格式的文件名应为：X(FD3).xlsx（X为数字）")
        return

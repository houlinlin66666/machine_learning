...
相对 v2 的唯一改动:
    预处理变为 "像素标准化 + PCA 降维":
      flatten(EEM) -> train_test_split -> StandardScaler -> PCA(n_components)
    所有多输出模型基于 PCA 主成分得分训练与预测;
    PCA 仅在训练集上 fit 避免泄露, 测试集仅 transform。
其余逻辑 (训练/评估/全部绘图) 全部与 v2 一致;
输出目录改名为 `figure_multi_pca / models_multi_pca / results_multi_pca.csv`
以避免覆盖 v2 结果。
...
FIGURE_DIR = os.path.join(DATA_DIR, "figure_multi_pca")
MODEL_DIR  = os.path.join(DATA_DIR, "models_multi_pca")
RESULT_CSV = os.path.join(DATA_DIR, "results_multi_pca.csv")
...
# -------- PCA 降维超参 (与 singleNongduPredit-pca.py 一致) --------
# float (0,1]  → 累计方差比阈值 (例 0.99 = 保留 99% 方差)
# int          → 直接指定主成分个数
...
def preprocess(X: np.ndarray, Y: np.ndarray):
    """
    1) flatten EEM 为 (N, n_ex*n_em)
    2) 按浓度 2D 分层划分 train/test
    3) 在训练集上 fit StandardScaler → 同步 transform 测试集
    4) 在标准化后的训练集上 fit PCA(PCA_N_COMPONENTS) → 同步 transform
    返回训练/测试主成分得分、Y、scaler、pca、特征名 (PC1..PCk)。
    """
    ...
    print(f"[Task 2] PCA: ({X_flat.shape[1]}) -> ({k})  cum_var={cum_var:.4f}")
...
def plot_feature_importance(models: Dict[str, Any],
    ...
    for name in ("RandomForest", "GradientBoosting", "XGBoost"):
        ...
        vals = imp[idx]
        fig, ax = plt.subplots(
            figsize=(SINGLE_COL * 2.0, 0.38 * top_n + 1.2))
        ...
        ax.set_title(f"Top-{top_n} principal components — {_abbr(name)} "
...
def plot_pca_variance(pca: PCA) -> None:
    ...
    ax.set_title(f"PCA explained variance (k = {k}, cum = {cum[-1]:.3f})")
...
def main() -> None:
    try:
        print("=" * 64)
        print("EEM Multi-Output Antibiotic Regression Pipeline (PCA)")
        print("=" * 64)
        ...
        meta_df.to_csv(os.path.join(DATA_DIR, "metadata_multi_pca.csv"),
                       index=False, encoding="utf-8-sig")

        X_tr, X_te, Y_tr, Y_te, scaler, pca, feature_names = preprocess(X, Y)
        ...
        plot_pca(X, Y)                                   # fig4b × N_TARGETS (原始 EEM PCA 二维可视化)
        plot_pca_variance(pca)                           # fig4c PCA 解释方差诊断

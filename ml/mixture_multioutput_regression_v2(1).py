# -*- coding: utf-8 -*-
"""
EEM Mixture Antibiotic — MULTI-OUTPUT Regression Pipeline (v2)
==================================================================
相对 v1 的改动:
    1. 散点图 (Measured vs Predicted): 每个 (抗生素 × 模型) 单独成图,
       训练集 #1F4E79, 测试集 #C0504D。
    2. R²/RMSE/MSE 柱状图改为 "所有模型" 版本:
         - fig3a_<target>_train_vs_test : 每种抗生素一张,
             X 轴=模型, 每模型两根柱 (Train #1F4E79, Test #C0504D)。
         - fig4_train_set_by_target     : 训练集,
             X 轴=模型, 每模型两根柱 (Ciprofloxacin #1F4E79,
                                       Ofloxacin     #C0504D)。
         - fig5_test_set_by_target      : 测试集, 同上配色。
    3. 其它逻辑 (数据读取/训练/评估/残差/误差曲线/特征重要度) 与 v1 一致。
...
FIGURE_DIR = os.path.join(DATA_DIR, "figure_multi_v2")
MODEL_DIR  = os.path.join(DATA_DIR, "models_multi_v2")
RESULT_CSV = os.path.join(DATA_DIR, "results_multi_v2.csv")
...
def main() -> None:
    try:
        print("=" * 64)
        print("EEM Multi-Output Antibiotic Regression Pipeline (v2)")
        print("=" * 64)
        ...
        meta_df.to_csv(os.path.join(DATA_DIR, "metadata_multi_v2.csv"),
        ...
        plot_pca(X, Y)                                   # fig4b × N_TARGETS
        plot_error_curves(results, Y_tr, Y_te)           # fig6
        plot_feature_importance(ml_models, ex_axis, em_axis, top_n=20)
